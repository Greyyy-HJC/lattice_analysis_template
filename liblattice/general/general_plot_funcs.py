...
def plot_ratio_fit_on_data_log(get_ratio_data, ra_fit_res, ra_re_fcn, ra_im_fcn, px, py, pz, b, z, err_tsep_ls, fill_tsep_ls, Ls, err_tau_cut=1, fill_tau_cut=1):
    """
...
def plot_sum_fit_on_data_log(get_sum_data, sum_fit_res, sum_re_fcn, sum_im_fcn, px, py, pz, b, z, err_tsep_ls, fill_tsep_ls, err_tau_cut=2, fill_tau_cut=2):
    """
    ...
    sum_re, sum_im = get_sum_data(px, py, pz, b, z, err_tsep_ls, jk_bs="bs", tau_cut=err_tau_cut)
    ...
    def plot_part(part, sum_avg, sum_fcn):
        ...
        errorbar_fill_between_ls_plot(
            x_ls, y_ls, yerr_ls, label_ls, plot_style_ls,
            title=f'Sum_fit_on_data_P{px}_{part}_b{b}_z{z}', save=False, head=ax
        )
        plt.savefig(f"../log/gsfit/Sum_fit_on_data_P{px}_{part}_b{b}_z{z}.pdf", transparent=True)
